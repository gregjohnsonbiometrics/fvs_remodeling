...
df_fit_ht40 <- minpack.lm::nlsLM( endDIA ~ est_dg_test_ht40( B0, B1, B2, B3, B4, B5, startDIA, startBAL, endBAL, startCR,
                 ...
                 start= c(    B0= -5.67,               
                              ...
                              B4 = 1.72,
                              B5 = 0.63 ), trace=T )
x <- summary( df_fit_ht40 )
knitr::kable( reformat_nls( x ) )
cat( paste( "Residual Standard Error:", x$sigma, "on", x$df[2], 
            "degrees of freedom, AIC:", AIC(df_fit_ht40) ))



dg_hat_ht40 <- predict( df_fit_ht40)
dg_res_ht40 <- ( dg_hat_ht40  - df_subset$endDIA ) / ( df_subset$endGROWYR - df_subset$startGROWYR )
 

df_subset %>% ggplot( aes( startDIA, dg_res_ht40) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~STATECD ) +
...
df_subset %>% ggplot( aes( startBAPA, dg_res_ht40) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~STATECD ) +    
...
df_subset %>% ggplot( aes( startBAL, dg_res_ht40) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~STATECD ) +    
...
df_subset %>% ggplot( aes( startCR, dg_res_ht40) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~STATECD ) +    
...
df_subset %>% ggplot( aes( SICOND, dg_res_ht40 ) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~as.factor(SIBASE) ) + 
...
df_subset %>% ggplot( aes( startHT40, dg_res_ht40) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~STATECD ) +    
...
df_subset %>% ggplot( aes( as.factor(STATECD), dg_res_ht40) ) + 
    geom_boxplot( notch = T, varwidth = T ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 ) + 
    xlab( "State Code") + 
