...
x <- df_subset %>% mutate( res=dg_res_nosi ) %>% group_by( LON, LAT ) %>% summarize( mean_res=mean( res ) )
...
x %>% ggplot( aes(x = LON, y = LAT, z = mean_res) ) + geom_contour_filled(alpha = 0.7) + scale_fill_distiller(palette = "Spectral", direction = -1)

x %>% ggplot( aes(x = LON, y = LAT, z = mean_res) ) +
    geom_contour_fill(na.fill = TRUE, breaks = MakeBreaks(binwidth = 0.02 )) 
