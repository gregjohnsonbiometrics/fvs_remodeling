...
$\Delta dbh = e^{\beta_0 + \beta_1 log(\frac{dbh^2 + 1}{cr * ht + 1}) + \beta_2 \frac{bal^2}{dbh+2.7} + \beta_3 log( si + 4.5) }$
...
est_dg_test <- function( B0, B1, B2, B3, dbh0, bal0, bal1, cr0, cr1, ht0, ht1, si, n )

{
    ...
    {
          dg_hat <- exp( B0 + 
                         B1 * log( cdbh^2/(ccr*cht+1.0) ) +
                         B2 * cbal^2/( log(cdbh+2.7) ) +
                         B3 * log( si + 4.5 ) ) 
...
df_fit_test <- minpack.lm::nlsLM( endDIA ~ est_dg_test( B0, B1, B2, B3, startDIA, startBAL, endBAL, startCR,
                                                endCR, startHT, endHT, SICOND, endGROWYR-startGROWYR ),
                 data = df_tree_subset_with_si,  
                 start= list( B0= -5.992437e+00, 
                              B1= -3.029155e-01, 
                              B2= -8.014774e-05,
                              B3=  1.077075  ), trace=F )
...
df_tree_subset_with_si %>% ggplot( aes( startDIA, dg_res) ) + 
    ...
    ylim( NA, 3 ) +
    xlab("dbh (inches)") +
    ylab( "Diameter Growth Residual (in/yr) (Pred-Act)") + ggtitle( "Alternative")
