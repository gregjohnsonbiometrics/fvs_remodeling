...
trees_df$csi <- si
tree_subset <- trees_df %>% filter( !is.na(csi) & !(State %in% c("NY", "PA", "IL", "MI")) )
...
old_fit <- minpack.lm::nlsLM( endDIA ~ est_dg( B0, B1, B2, 0.0, B4, B5, startDIA, startBAL, endBAL, startCR,
...
tree_subset %>% ggplot( aes( startBAL, dg_res) ) + 
    ...
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~Hmisc::cut2( csi, g=4 ) ) +    
    ylim( -0.5, 0.5 )+
    xlab("bal (ft^2/acre)") +
    ylab( "Diameter Growth Residual (in/yr) (Pred-Act)") + ggtitle( "Diameter Growth Model with Climate SI", 
                                                                    subtitle = "Outliers Removed, Paneled by Climate SI" )
