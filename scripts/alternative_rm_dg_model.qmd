---
title: "Alternative Red Maple Diameter Growth"
...
states <- c("AL","AR","CT","DE","FL","GA","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","NE","NH",
            "NJ","NY","NC","OH","OK","PA","RI","SC","TN","TX","VT","VA","WV","WI")
...
tree_subset <- tree_data %>% 
    filter( SPCD == fia_species &
            !is.na(startDIA) & !is.na(startBAPA) & !is.na(startBAL) & !is.na(startCR) & !is.na(startHT) & !is.na( endHT ) &
...
si_fit <- nlme( endDIA ~ est_dg( B0, B1, B2, B3, B4, B5, startDIA, startBAL, endBAL, startCR,
                 ...
                 data = tree_subset %>% mutate( SIINT=interaction(as.factor( tree_subset$SIBASE),as.factor(tree_subset$SISP == fia_species))), 
                 ...
                 start= c(    B0= -4.720055,                
                              ...
                              B3=  0.211395,
                              B4 = 3.164884,
                              B5 = 0.7012752 ) ) #, trace=F )

# si_fit <- nls( endDIA ~ est_dg( B0, B1, B2, B3, B4, B5, startDIA, startBAL, endBAL, startCR,
#                                  endCR, startHT, endHT, SICOND, endGROWYR-startGROWYR ),
#                  data = tree_subset, 
#                  start= c(    B0= -4.720055,                
#                               B1= -0.3783203, 
...
tree_subset %>% ggplot( aes( SICOND, dg_res ) ) + 
    ...
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~ifelse(SISP == fia_species, paste(species_name,"si"),paste("non", species_name, "si") ) ) + 
    ylim( -1, 1 )+
    xlab("si (feet)") + 
...
Removing `si` does not degrade the fit significantly. @eq-nosite residual graphs show that there are some residual trends spatially.
...
eco_regions <- st_read(paste0(gis_path, "ecoregions.gpkg"))

tree_subset_sf <- st_as_sf(tree_subset, coords = c("LON", "LAT"), crs = 4326) 
tree_subset_sf <- st_transform(tree_subset_sf, st_crs(eco_regions))

tree_subset_sf_eco <- st_join(tree_subset_sf, eco_regions, join = st_intersects)

tree_subset_sf_eco %>% ggplot( aes( NA_L2NAME, dg_res_nosi) ) + 
    geom_boxplot( notch = T, varwidth = T ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    theme(axis.text.x = element_text(angle = -15, vjust = 1, hjust = 0)) +
    ylim(-0.5,0.5) +
    xlab( "Level II Eco Region" ) + ylab( "Diameter Growth Residual (in/yr) (Pred-Act)") +
    ggtitle( paste( species_name, " No Site Diameter Growth Residuals"), subtitle= "Outliers Removed" )
```
