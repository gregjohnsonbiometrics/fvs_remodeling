---
title: "Alternative Balsam Fir Pine Diameter Growth"
...
states <- c( "ME", "NH", "VT", "MN",  "MD", "WV", "NJ", "VA", "CT", "MA" ) #  "WI","MI","NY", 

fia_species <- 12
species_name <- "Balsam Fir"
...
si_fit <- nlme( endDIA ~ est_dg( B0, B1, B2, B3, B4, B5, startDIA, startBAL, endBAL, startCR,
                 ...
                 data = tree_subset %>% mutate( SIINT=interaction(as.factor( tree_subset$SIBASE),as.factor(tree_subset$SISP == 12))), 
...
tree_subset %>% ggplot( aes( SICOND, dg_res ) ) + 
    ...
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    facet_wrap( ~ifelse(SISP == 12, "BF si","non BF si") ) + 
    ylim( -1, 1 )+
    xlab("si (feet)") + 
...
Removing `si` does not degrade the fit significantly. Only MN appears to be over-predicted. 

@eq-withsi residual graphs show that there is some indication of residual trends spatially (especially in the western states).
