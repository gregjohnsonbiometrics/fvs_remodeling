---
title: "Alternative Western Hemlock Diameter Growth"
...
wh_states <- c( "WA", "OR", "CA", "MT", "ID" ) 

# get change data
wh_tree_data <- NULL
for( state in wh_states )
    wh_tree_data <- rbind( wh_tree_data,  read.csv( paste0(data_path, state, "_CHANGEdata.CSV"), header=T, sep="," ) )

   
wh_subset <- wh_tree_data %>% 
    filter( startSPCD == 263 &
...
We extracted and processed Forest Inventory and Analysis (FIA) data from `r length(unique(wh_subset$STATECD))` states listed in the native range of Western Hemlock in the Silvics of North America.[^1]
...
After subsetting the data to censor observations with missing data, limiting the species to Western Hemlock (FIA species code 263), and remeasurement intervals $\ge$ 5 years we get the observations in Table \ref{tbl:statesum}.
...
x <- wh_subset %>% group_by( State ) %>% summarise( n = length(startDIA))

knitr::kable( x[,c("State","n")], row.names = F, col.names = c("State", "Observations"),
              caption = "\\label{tbl:statesum} Western Hemlock Growth Observations by State")
```
...
[^2]: Hann, D.W., Marshall, D.D., and Hanus, M.L. 2006. Reanalysis of the SMC-ORGANON equations for diameter-growth rate, height-growth rate, and mortality rate of Western Hemlock. Forest Research Laboratory Research Contribution 49.

Site index is flawed for a number of reasons:

1.  It is not consistently obtained for each plot due to missing Western Hemlock site trees,
...
In the data set `si` is derived from `r length(unique(wh_subset$SIEQN_REF_CD_FVS))` different site index equations for `r length(unique(wh_subset$SISP))` species. Western Hemlock site index comprises `r round(100*nrow(wh_subset %>% filter( SISP == 263 ))/nrow(wh_subset),0)`% of the observations. There are `r length(unique(wh_subset$SIBASE))` base ages used. Preliminary graphical analysis revealed that base age was most correlated with residual bias. Thus in the following, we fit two equations: one where `SIBASE` and `SISP` are treated as a random effects in a mixed model framework, and a second leaving site index out.
...
Nonlinear regression was used with an integrated fitting awhroach such that individual observations can have differing remeasurement intervals. The error to be minimized is ending `dbh`. Since this effectively minimizes diameter growth it can weight observations with longer remeasurement intervals more heavily. The effect of this needs to be evaluated, but putting more emphasis on longer periods may be beneficial.
...
wh_fit <- nlme( endDIA ~ est_dg( B0, B1, B2, B3, B4, B5, startDIA, startBAL, endBAL, startCR,
                                 endCR, startHT, endHT, SICOND, endGROWYR-startGROWYR ),
                 data = wh_subset %>% mutate( SIINT=interaction(as.factor( wh_subset$SIBASE),as.factor(wh_subset$SISP == 202))), 
...
cat( paste( "Residual Standard Error:", x$sigma, "on", x$fixDF$X[1], #x$df[2], 
            "degrees of freedom, AIC:", round(AIC(wh_fit),1) ))
...
wh_fit_nosi <- minpack.lm::nlsLM( endDIA ~ est_dg( B0, B1, B2, 0.0, B4, B5, startDIA, startBAL, endBAL, startCR,
                                                   endCR, startHT, endHT, SICOND, endGROWYR-startGROWYR ),
                 data = wh_subset, 
...
x <- summary( wh_fit_nosi )
knitr::kable( reformat_nls( x ) )
cat( paste( "Residual Standard Error:", x$sigma, "on", x$df[2], 
            "degrees of freedom, AIC:", round(AIC(wh_fit_nosi),1) ))
...
dg_hat <- predict( wh_fit)
dg_res <- ( dg_hat  - wh_subset$endDIA ) / ( wh_subset$endGROWYR - wh_subset$startGROWYR )

wh_subset %>% ggplot( aes( startDIA, dg_res) ) + 
    geom_point() + 
...
wh_subset %>% ggplot( aes( startBAPA, dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( startBAL, dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( startCR, dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( SICOND, dg_res ) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
...
wh_subset %>% ggplot( aes( startHT40, dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( as.factor(State), dg_res) ) + 
...
wh_subset %>% ggplot( aes( LAT, dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
wh_subset %>% ggplot( aes( LON, dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
wh_subset %>% ggplot( aes( SLOPE/100 * cos(ASPECT*pi/180), dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
wh_subset %>% ggplot( aes( SLOPE/100 * sin(ASPECT*pi/180), dg_res) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
dg_hat_nosi <- predict( wh_fit_nosi)
dg_res_nosi <- ( dg_hat_nosi  - wh_subset$endDIA ) / ( wh_subset$endGROWYR - wh_subset$startGROWYR )
 

wh_subset %>% ggplot( aes( startDIA, dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +
...
wh_subset %>% ggplot( aes( startBAPA, dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( startBAL, dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( startCR, dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( SICOND, dg_res_nosi ) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
...
wh_subset %>% ggplot( aes( startHT40, dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    # facet_wrap( ~State ) +    
...
wh_subset %>% ggplot( aes( as.factor(State), dg_res_nosi) ) + 
    geom_boxplot( notch = T, varwidth = T ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 ) + 
    xlab( "State Code") + 
...
wh_subset %>% ggplot( aes( LAT, dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
wh_subset %>% ggplot( aes( LON, dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
wh_subset %>% ggplot( aes( SLOPE/100 * cos(ASPECT*pi/180), dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
wh_subset %>% ggplot( aes( SLOPE/100 * sin(ASPECT*pi/180), dg_res_nosi) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    geom_abline( intercept = 0, slope = 0 ) +
    ylim( -0.5, 0.5 )+ 
...
x <- wh_subset %>% mutate( res=dg_res_nosi ) %>% group_by( LON, LAT ) %>% summarize( mean_res=mean( res ) )
...
wh_subset %>% ggplot( aes( startDIA, (dg_hat-startDIA)/( endGROWYR - startGROWYR )) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    xlab("dbh (inches)") +
    ylab( "Predicted Diameter Growth (in/yr)") + ggtitle( "Alternative")

wh_subset %>% ggplot( aes( startBAPA, (dg_hat-startDIA)/( endGROWYR - startGROWYR ) ) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    xlab("ba (ft^2/acre)") +
    ylab( "Predicted Diameter Growth (in/yr)") + ggtitle( "Alternative")

wh_subset %>% ggplot( aes( startBAL, (dg_hat-startDIA)/( endGROWYR - startGROWYR ) ) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    xlab("bal (ft^2/acre)") +
    ylab( "Predicted Diameter Growth (in/yr)") + ggtitle( "Alternative")

wh_subset %>% ggplot( aes( startCR, (dg_hat-startDIA)/( endGROWYR - startGROWYR ) ) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    xlab("cr (fraction of ht)") +
    ylab( "Predicted Diameter Growth (in/yr)") + ggtitle( "Alternative")

wh_subset %>% ggplot( aes( SICOND, (dg_hat-startDIA)/( endGROWYR - startGROWYR ) ) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    facet_wrap( ~as.factor(SIBASE)) +
    xlab("si (feet)") +
...
wh_subset %>% ggplot( aes( startHT40,  (dg_hat-startDIA)/( endGROWYR - startGROWYR )) ) + 
    geom_point() + 
    geom_smooth( method = 'gam',   formula = y ~ s(x, bs = "cs") ) + 
    xlab( "ht40 (feet)") +
    ylab( "Predicted Diameter Growth (in/yr)") + ggtitle( "Alternative")
...
Removing `si` degrades the fit significantly. There is an over-prediction bias for the eastern states for both equations, but is mitigated by the `si` term in @eq-withsi. The residual trend with longitude confirms this.
