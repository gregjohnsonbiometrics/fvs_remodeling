...
saveRDS( fit_stats, file=paste0(rds_path,"dg_parms_elev_emt.RDS") )
...
#| include: false
#| 
fit_stats <- readRDS( file=paste0(rds_path,"dg_parms_elev_emt.RDS") )
...
$$\Delta dbh = e^{(\beta_0 + \beta_1 log(\frac{(dbh+1)^2}{(cr * ht + 1)^{\beta_3}}) + \beta_2 \frac{bal^{\beta_4}}{dbh+2.7} + \beta_5 elev + \beta_6 EMT) }$$ {#eq-dg}
...
fit_stats2 %>% ggplot( aes( type, B5 )) + geom_violin(draw_quantiles=c(0.25,0.5,0.75)) + xlab("")+
    ggtitle( "Elevation above Sea Level" ) 
