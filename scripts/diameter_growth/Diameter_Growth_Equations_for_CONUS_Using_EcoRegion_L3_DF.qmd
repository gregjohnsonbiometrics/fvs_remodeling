...
tree_subset <- x |> inner_join( tree_subset, by=c("PLOT", "STATECD","UNITCD", "COUNTYCD" )) %>% 
  filter( !(State %in% c("NY","PA","IL","MI")) & !is.na(NA_L3NAME) & endDIA > startDIA )
...
The residual analysis below shows that state and L3 Ecoregion bias has been mitigated but not eliminated. It also shows that the trends with the traditional site index estimates have been reduced.
