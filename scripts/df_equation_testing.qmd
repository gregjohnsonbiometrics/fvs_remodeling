---
title: "FVS Remodel Project: Oregon Douglas-fir FIA Data -- Do Site Index and Basal Area per Acre Matter?"
...
df_tree_data <- read.csv( paste0(data_path, "OR_CHANGEdata_DF.CSV"), header=T, sep="," )

   
df_tree_subset <- df_tree_data %>% 
    filter( !is.na(startDIA) & !is.na(startBAPA) & !is.na(startBAL) & !is.na(startCR) &
...
We took the `OR_CHANGEdata_DF` Douglas-fir data set provided by David Marshall as a starting point. These data are remeasurement observations from Forest Inventory and Analysis (`FIA`) plots in Oregon.

We subsetted the data to drop observations with missing values for diameter at breast height (`dbh`), basal area per acre (`ba`), basal area per acre in larger trees (`bal`), crown ratio (`cr`) expressed as a fraction, and site index (`si`) at the beginning and ending measurement points. We also dropped remeasurements with measurement intervals less than 5 years.
...
df_tree_subset_with_si %>% ggplot( aes( startDIA, dg_res) ) + 
    ...
    ylim( NA, 4 ) +
    xlab("dbh (inches)") +
    ylab( "Diameter Growth Residual (in/yr) (Pred-Act)") + ggtitle( "With Site Index")
...
df_tree_subset_with_si %>% ggplot( aes( startDIA, dg_res) ) + 
    ...
    ylim( NA, 4 ) +
    xlab("dbh (inches)") +
    ylab( "Diameter Growth Residual (in/yr) (Pred-Act)") + ggtitle( "Without Site Index")
...
df_tree_subset_with_si %>% ggplot( aes( startDIA, dg_res) ) + 
    ...
    ylim( NA, 4 ) +
    xlab("dbh (inches)") +
    ylab( "Diameter Growth Residual (in/yr) (Pred-Act)") + ggtitle( "Without Basal Area")
...
```
The nested model F test indicates that adding the `ba` variable is significant; however, the AIC comparisons indicate that `ba` is not adding much explanatory value to the model. It appears that we do not pay a large penalty for dropping `ba` and we gain biological consistency.
